...
method TestsForReverseUptoK()
{
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    ...
    // expect s[..] == [-9, -10]; // LHS=[-10, -10], RHS=[-9, -10]
  }
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    ...
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[-9, -9]
    // expect s[..] == [-8, -9]; // LHS=[-9, -9], RHS=[-8, -9]
  }
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    ...
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[5, 5]
    // expect s[..] == [8, 5]; // LHS=[5, 5], RHS=[8, 5]
  }
