...
method TestsForDissimilarElements()
{
  ...
  //   POST Q1: forall x: int {:trigger InArray(b, x)} {:trigger InArray(a, x)} {:trigger x in result} :: x in result ==> InArray(a, x) != InArray(b, x)
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]  // VACUOUS (forced true by other literals for this ins)
  {
    var a := new int[0] [];
    var b := new int[0] [];
    var result := DissimilarElements(a, b);
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    // runtime error: at _module.__default.DissimilarElements(BigInteger[] a, BigInteger[] b) in C:\cygwin64\tmp\DafnyCBT_2wcqomluuwj\runner.cs:line 5929
    // runtime error: at _module.__default.TestCase__3() in C:\cygwin64\tmp\DafnyCBT_2wcqomluuwj\runner.cs:line 6111
  ...
  {
    var a := new int[0] [];
    var b := new int[3] [9, 11, 11];
    var result := DissimilarElements(a, b);
    expect forall x: int  :: x in result ==> InArray(a, x) != InArray(b, x);
    expect forall i: int, j: int :: 0 <= i < j < |result| ==> result[i] != result[j];
    expect result == [9, 11]; // observed from implementation
  }
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    var b := new int[1] [34];
    var result := DissimilarElements(a, b);
    expect result == [] || result == [34];
    expect result[..] == [34]; // observed from implementation
  }
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    var b := new int[1] [36];
    var result := DissimilarElements(a, b);
    expect result == [] || result == [36];
    expect result[..] == [36]; // observed from implementation
  }
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    var b := new int[1] [32];
    var result := DissimilarElements(a, b);
    expect result == [] || result == [32];
    expect result[..] == [32]; // observed from implementation
  }
