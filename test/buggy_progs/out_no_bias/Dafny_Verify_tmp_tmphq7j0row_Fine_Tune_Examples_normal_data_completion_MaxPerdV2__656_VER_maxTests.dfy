...
method TestsFormax()
{
  ...
  //   POST Q2: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max  // VACUOUS (forced true by other literals for this ins)
  {
    ...
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__2() in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 6074
    // expect max == 0;
  }
  ...
  //   POST Q1: is_max(max, a, n)
  //   POST Q2: a[(n - 1)] == max
  //   POST Q3: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max  // VACUOUS (forced true by other literals for this ins)
  {
    var a := new int[1] [0];
    var n := 1;
    var max := max(a, n);
  ...
  //   POST Q2: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    ...
    // runtime error: Unhandled exception. System.IndexOutOfRangeException: Index was outside the bounds of the array.
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__6() in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 6211
    // expect max == 0;
  }
  ...
  //   POST Q3: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    ...
    // runtime error: Unhandled exception. System.IndexOutOfRangeException: Index was outside the bounds of the array.
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__7() in C:\cygwin64\tmp\DafnyCBT_rkpsorqeyut\runner.cs:line 6245
    // expect max == 175;
  }
