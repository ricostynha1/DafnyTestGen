...
method TestsForReplaceBlanksWithChar()
{
  ...
  //   POST Q2: forall i: int {:trigger v[i]} {:trigger s[i]} :: (0 <= i < |s| ==> s[i] == ' ' ==> v[i] == ch) && (0 <= i < |s| ==> s[i] != ' ' ==> v[i] == s[i])
  {
    ...
    // runtime error: at Dafny.Sequence`1.Select(BigInteger index) in C:\cygwin64\tmp\DafnyCBT_wtda05cyk5a\runner.cs:line 1510
    // expect v == ['q'];
  }

  // Test case for combination {1}/V2:
  //   POST Q1: |v| == |s|
  //   POST Q2: forall i: int {:trigger v[i]} {:trigger s[i]} :: (0 <= i < |s| ==> s[i] == ' ' ==> v[i] == ch) && (0 <= i < |s| ==> s[i] != ' ' ==> v[i] == s[i])  // VACUOUS (forced true by other literals for this ins)
  {
    var s: seq<char> := [];
    var ch := ' ';
    var v := ReplaceBlanksWithChar(s, ch);
