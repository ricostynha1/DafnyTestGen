...
method TestsForDeli()
{
  ...
  //   PRE:  a.Length > 0
  //   PRE:  0 <= i < a.Length
  //   POST Q1: forall j: int {:trigger old(a[j])} {:trigger a[j]} :: 0 <= j < i ==> a[j] == old(a[j])  // VACUOUS (forced true by other literals for this ins)
  //   POST Q2: forall j: int {:trigger a[j]} :: i <= j < a.Length - 1 ==> a[j] == old(a[j + 1])
  //   POST Q3: a[a.Length - 1] == '.'
  {
    var a := new char[1] ['/'];
  ...
  //   POST Q3: a[a.Length - 1] == '.'
  {
    ...
    Deli(a, i);
    expect a[..] == ['h', ' ', '.'];
    expect a[..] == h .; // observed from implementation
  }
