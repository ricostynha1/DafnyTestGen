...
method TestsForFindAddends()
{
  ...
  //   POST Q1: i < j
  //   POST Q2: j < |q|
  //   POST Q3: q[i] + q[j] == x  // VACUOUS (forced true by other literals for this ins)
  {
    var q: seq<int> := [-10, 9];
    var x := -1;
    var i, j := FindAddends(q, x);
  ...
  //   POST Q3: q[i] + q[j] == x
  {
    var q: seq<int> := [4, 5, 5, 5];
    var x := 10;
    var i, j := FindAddends(q, x);
    expect i == 2 || i == 1 || i == 1;
    expect j == 3 || j == 2 || j == 3;
    expect i == 1; // observed from implementation
    expect j == 2; // observed from implementation
  }
