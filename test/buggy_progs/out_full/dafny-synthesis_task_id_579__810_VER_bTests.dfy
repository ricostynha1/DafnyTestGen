...
method TestsForDissimilarElements()
{
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    // runtime error: at _module.__default.DissimilarElements(BigInteger[] a, BigInteger[] b) in C:\cygwin64\tmp\DafnyCBT_0e5bckdb3we\runner.cs:line 5946
    // runtime error: at _module.__default.TestCase__3() in C:\cygwin64\tmp\DafnyCBT_0e5bckdb3we\runner.cs:line 6108
    // expect forall x: int  :: x in result ==> InArray(a, x) != InArray(b, x);
    // expect forall i: int, j: int :: 0 <= i < j < |result| ==> result[i] != result[j];
  }
  ...
  //   POST Q2: forall i: int, j: int {:trigger result[j], result[i]} :: 0 <= i < j < |result| ==> result[i] != result[j]
  {
    ...
    // runtime error: Unhandled exception. System.IndexOutOfRangeException: Index was outside the bounds of the array.
    // runtime error: at _module.__default.DissimilarElements(BigInteger[] a, BigInteger[] b) in C:\cygwin64\tmp\DafnyCBT_0e5bckdb3we\runner.cs:line 5946
    // runtime error: at _module.__default.TestCase__4() in C:\cygwin64\tmp\DafnyCBT_0e5bckdb3we\runner.cs:line 6161
    // expect result == [] || result == [-10];
  }
