...
// Source: C:\Dados\Dafny\DafnyTestGen\test\buggy_progs\in\BPTree-verif_tmp_tmpq1z6xm1d_Utils__11949_VER_key.dfy
// Method: GetInsertIndex
// Generated: 2026-04-24 09:25:27
...
method TestsForInsertIntoSorted()
{
  ...
  //   POST Q5: forall i: int {:trigger b[i]} :: 0 <= i < limit + 1 ==> b[i] > 0
  {
    ...
    expect b[..] == [5, 6, 9, 10, 11, 17, 0, 0]; // observed from implementation
  }
  ...
  //   POST Q5: forall i: int {:trigger b[i]} :: 0 <= i < limit + 1 ==> b[i] > 0
  {
    ...
    var key := 1;
    var b := InsertIntoSorted(a, limit, key);
    // actual runtime state: b=[1, 6, 0, 0]
    ...
    // expect forall i: int :: 0 <= i < limit ==> a[i] in b[..]; // got false
    // expect forall i: int :: 0 <= i < limit + 1 ==> b[i] > 0; // got false
  }
  ...
  //   POST Q5: forall i: int {:trigger b[i]} :: 0 <= i < limit + 1 ==> b[i] > 0
  {
    ...
    var key := 2;
    var b := InsertIntoSorted(a, limit, key);
    // actual runtime state: b=[2, 7, 8, 0]
    // expect b.Length == a.Length; // LHS=4, RHS=4
    // expect sorted(b[..limit + 1]); // got false
    // expect forall i: int :: limit + 1 <= i < b.Length ==> b[i] == 0; // got true
    // expect forall i: int :: 0 <= i < limit ==> a[i] in b[..]; // got false
