...
method TestsForSearchRecursive()
{
  ...
  //   POST Q3: k != -1
  //   POST Q4: a[k] == x
  //   POST Q5: forall r: int {:trigger a[r]} | k < r < j :: a[r] != x  // VACUOUS (forced true by other literals for this ins)
  {
    var a: seq<int> := [2, 2, 2];
    var i := 2;
    var j := 3;
  ...
  // Test case for combination {3}/O|a|=0:
  //   PRE:  0 <= i <= j <= |a|
  //   POST Q1: i > k
  //   POST Q2: k == -1
  //   POST Q3: forall r: int {:trigger a[r]} | i <= r < j :: a[r] != x
...
method TestsForSearchLoop()
{
  ...
  //   POST Q5: forall r: int {:trigger a[r]} | k < r < j :: a[r] != x
  {
    ...
    // runtime error: at Dafny.Sequence`1.Select(BigInteger index) in C:\cygwin64\tmp\DafnyCBT_gocubpnekww\runner.cs:line 1642
    // expect k == 0;
  }
