...
method TestsForthreshold()
{
  ...
  {
    var thres := -2;
    var Seq: seq<int> := [21, 20];
    var Seq_1, Seq_2 := threshold(thres, Seq);
    // actual runtime state: Seq_1=[21, 20], Seq_2=[]
    // expect (forall x: int | x in Seq_1 :: x <= thres) && forall x: int | x in Seq_2 :: x >= thres; // got false
    // expect |Seq_1| + |Seq_2| == |Seq|; // LHS=2, RHS=2
    // expect multiset(Seq_1) + multiset(Seq_2) == multiset(Seq); // LHS=multiset{20, 21}, RHS=multiset{20, 21}
  }
  ...
  //   POST Q3: multiset(Seq_1) + multiset(Seq_2) == multiset(Seq)
  {
    ...
    var Seq: seq<int> := [32];
    var Seq_1, Seq_2 := threshold(thres, Seq);
    // actual runtime state: Seq_1=[32], Seq_2=[]
    // expect (forall x: int | x in Seq_1 :: x <= thres) && forall x: int | x in Seq_2 :: x >= thres; // got false
    // expect |Seq_1| + |Seq_2| == |Seq|; // LHS=1, RHS=1
    // expect multiset(Seq_1) + multiset(Seq_2) == multiset(Seq); // LHS=multiset{32}, RHS=multiset{32}
  }
