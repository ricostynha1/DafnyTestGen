...
method TestsFormax()
{
  ...
  //   POST Q2: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    ...
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__1() in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 6038
    // expect max == 0;
  }
  ...
  //   POST Q2: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    var a := new int[4] [-576, -576, -577, 25];
    var n := 3;
    var max := max(a, n);
    expect max == -576;
  }
  ...
  //   POST Q3: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    ...
    // runtime error: Unhandled exception. System.IndexOutOfRangeException: Index was outside the bounds of the array.
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__5() in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 6176
    // expect max == 175;
  }
  ...
  //   POST Q2: forall j: int {:trigger a[j]} :: 0 <= j && j < n ==> a[j] <= max
  {
    var a := new int[3] [-400, 17870, -401];
    var n := 3;
    var max := max(a, n);
    // runtime error: Unhandled exception. System.IndexOutOfRangeException: Index was outside the bounds of the array.
    // runtime error: at _module.__default.max(BigInteger[] a, BigInteger n) in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 5971
    // runtime error: at _module.__default.TestCase__7() in C:\cygwin64\tmp\DafnyCBT_1ffluut4sv5\runner.cs:line 6244
    // expect max == 17870;
  }
