...
method TestsForReverseUptoK()
{
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    var s := new int[3] [16, 10, 34];
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[16, 16, 34]
    // expect s[..] == [10, 16, 34]; // LHS=[16, 16, 34], RHS=[10, 16, 34]
  }
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    var s := new int[2] [20, 14];
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[20, 20]
    // expect s[..] == [14, 20]; // LHS=[20, 20], RHS=[14, 20]
  }
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    var s := new int[2] [22, 15];
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[22, 22]
    // expect s[..] == [15, 22]; // LHS=[22, 22], RHS=[15, 22]
  }
  ...
  //   POST Q2: forall i: int {:trigger old(s[i])} {:trigger s[i]} :: k <= i < s.Length ==> s[i] == old(s[i])
  {
    var s := new int[2] [24, 17];
    var k := 2;
    ReverseUptoK(s, k);
    // actual runtime state: s=[24, 24]
    // expect s[..] == [17, 24]; // LHS=[24, 24], RHS=[17, 24]
  }
