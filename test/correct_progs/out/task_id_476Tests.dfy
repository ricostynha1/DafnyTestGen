// Auto-generated test cases by DafnyTestGen
// Source: C:\Dados\Dafny\DafnyTestGen\test\correct_progs\in\task_id_476.dfy
// Method: SumMinMax
// Generated: 2026-04-10 23:38:55

// Returns the sum of the minimum and maximum elements of a non-empty array
method SumMinMax(a: array<int>) returns (sum: int)
  requires a.Length > 0
  ensures sum == Max(a[..]) + Min(a[..])
{
  var minVal := a[0];
  var maxVal := a[0];
  for i := 1 to a.Length
    invariant maxVal == Max(a[..i])
    invariant minVal == Min(a[..i])
  {
    assert a[..i+1] == a[..i] + [a[i]]; // proof helper
    if a[i] < minVal {
      minVal := a[i];
    } 
    if a[i] > maxVal {
      maxVal := a[i];
    }
  }
  sum := minVal + maxVal;
  assert a[..a.Length] == a[..]; // proof helper
}

// Auxiliary function that gives the minimum element of a non-empty sequence.
function {:fuel 4} Min(a: seq<int>) : int
  requires |a| > 0
{
  if |a| == 1 then a[0]
  else if Last(a) <= Min(DropLast(a)) then Last(a) else Min(DropLast(a))
}

// Auxiliary function that gives the maximum element of a non-empty sequence.
function {:fuel 4} Max(a: seq<int>) : int
  requires |a| > 0
{
  if |a| == 1 then a[0]
  else if Last(a) >= Max(DropLast(a)) then Last(a) else Max(DropLast(a))
}

// Auxiliary function that gives the last element of a non-empty sequence
function Last<T>(a: seq<T>): T
  requires |a| > 0
{
  a[|a| - 1]
}

// Auxiliary function that gives a sequence without the last element.
function DropLast<T>(a: seq<T>): seq<T>
  requires |a| > 0
{
  a[0..|a| - 1]
}

// Test cases checked statically.
method SumMinMaxTest(){
  var a1 := new int[] [1,2,3];
  var out1 := SumMinMax(a1);
  assert out1 == 4;

  var a2 := new int[] [-1,2,3,4];
  var out2 := SumMinMax(a2);
  assert out2 == 3;

  var a3 := new int[] [2,3,6];
  var out3 := SumMinMax(a3);
  assert out3 == 8;
}


method Passing()
{
  // Test case for combination {1}:
  //   PRE:  a.Length > 0
  //   POST: sum == Max(a[..]) + Min(a[..])
  //   POST: sum == a[..][0]
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[1] [2];
    var sum := SumMinMax(a);
    expect sum == 4;
  }

  // Test case for combination {2}:
  //   PRE:  a.Length > 0
  //   POST: !(|a[..]| == 1)
  //   POST: sum == if (a[..][|a[..]| - 1]) >= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))) + (if |a[..]| == 1 then a[..][0] else if (a[..][|a[..]| - 1]) <= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[2] [3, 4];
    var sum := SumMinMax(a);
    expect sum == 7;
  }

  // Test case for combination {2}/Ba=3:
  //   PRE:  a.Length > 0
  //   POST: !(|a[..]| == 1)
  //   POST: sum == if (a[..][|a[..]| - 1]) >= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))) + (if |a[..]| == 1 then a[..][0] else if (a[..][|a[..]| - 1]) <= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[3] [5, 4, 6];
    var sum := SumMinMax(a);
    expect sum == 10;
  }

  // Test case for combination {2}/Osum>0:
  //   PRE:  a.Length > 0
  //   POST: !(|a[..]| == 1)
  //   POST: sum == if (a[..][|a[..]| - 1]) >= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))) + (if |a[..]| == 1 then a[..][0] else if (a[..][|a[..]| - 1]) <= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[4] [5, 6, 7, 8];
    var sum := SumMinMax(a);
    expect sum == 13;
  }

  // Test case for combination {2}/Osum<0:
  //   PRE:  a.Length > 0
  //   POST: !(|a[..]| == 1)
  //   POST: sum == if (a[..][|a[..]| - 1]) >= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))) + (if |a[..]| == 1 then a[..][0] else if (a[..][|a[..]| - 1]) <= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[5] [6, 7, 8, 9, 10];
    var sum := SumMinMax(a);
    expect sum == 16;
  }

  // Test case for combination {2}/Osum=0:
  //   PRE:  a.Length > 0
  //   POST: !(|a[..]| == 1)
  //   POST: sum == if (a[..][|a[..]| - 1]) >= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) >= Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Max(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))) + (if |a[..]| == 1 then a[..][0] else if (a[..][|a[..]| - 1]) <= (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1]))) then (a[..][|a[..]| - 1]) else (if |(a[..][0 .. |a[..]| - 1])| == 1 then (a[..][0 .. |a[..]| - 1])[0] else if ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) <= Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])) then ((a[..][0 .. |a[..]| - 1])[|(a[..][0 .. |a[..]| - 1])| - 1]) else Min(((a[..][0 .. |a[..]| - 1])[0 .. |(a[..][0 .. |a[..]| - 1])| - 1])))
  //   ENSURES: sum == Max(a[..]) + Min(a[..])
  {
    var a := new int[6] [7, 8, 9, 10, 11, 12];
    var sum := SumMinMax(a);
    expect sum == 19;
  }

}

method Failing()
{
  // (no failing tests)
}

method Main()
{
  Passing();
  Failing();
}
