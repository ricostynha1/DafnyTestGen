...
method TestsForFindFirstRepeatedChar()
{
  ...
  //   ENSURES: found ==> exists i: int, j: int :: 0 <= i < j < |s| && s[i] == s[j] == c && forall k: int, l: int :: 0 <= k < i && k < l < |s| ==> s[k] != s[l]
  //   ENSURES: !found ==> forall i: int, j: int :: 0 <= i < j < |s| ==> s[i] != s[j]
  {
    var s: seq<char> := [];
  ...
  //   POST: found
  //   POST: exists i: int, j: int :: 0 <= i < j < |s| && s[i] == s[j] == c && forall k: int, l: int :: 0 <= k < i && k < l < |s| ==> s[k] != s[l]
  //   ENSURES: found ==> exists i: int, j: int :: 0 <= i < j < |s| && s[i] == s[j] == c && forall k: int, l: int :: 0 <= k < i && k < l < |s| ==> s[k] != s[l]
  //   ENSURES: !found ==> forall i: int, j: int :: 0 <= i < j < |s| ==> s[i] != s[j]
  {
    var s: seq<char> := ['~', '~'];
