...
method Passing()
{
  ...
  //   ENSURES: result <==> forall i :: 0 <= i < |a| ==> a[i] < b[i]
  {
    var a: seq<int> := [16, 17, 18, 7719, 40];
    var b: seq<int> := [16, 17, 18, 7719, 40];
    var result := IsSmaller(a, b);
    expect result == false;
  }
...
method Failing()
{
  ...
  //   PRE:  |a| == |b|
  //   POST: !result
  //   POST: exists i, i_2 :: 0 <= i < i_2 <= (|a| - 1) && !(a[i] < b[i]) && !(a[i_2] < b[i_2])
  //   ENSURES: result <==> forall i :: 0 <= i < |a| ==> a[i] < b[i]
  {
    var a: seq<int> := [9, 8];
    var b: seq<int> := [10, 11];
