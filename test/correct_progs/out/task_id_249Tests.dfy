...
method Passing()
{
  ...
  //   POST: forall k: int {:trigger res[k]} :: (0 <= k < |res| ==> res[k] in a[..]) && (0 <= k < |res| ==> res[k] in b[..])
  //   POST: forall x: T {:trigger x in res} {:trigger x in b[..]} {:trigger x in a[..]} :: x in a[..] && x in b[..] ==> x in res
  //   POST: forall p: int, q: int {:trigger res[q], res[p]} :: 0 <= p < q < |res| ==> res[p] != res[q]
  //   POST: forall p: int, q: int {:trigger res[q], res[p]} :: 0 <= p < q < |res| ==> exists i: int, j: int {:trigger a[j], a[i]} :: 0 <= i < j < a.Length && a[i] == res[p] && a[j] == res[q]
  //   ENSURES: forall k: int {:trigger res[k]} :: (0 <= k < |res| ==> res[k] in a[..]) && (0 <= k < |res| ==> res[k] in b[..])
  //   ENSURES: forall x: T {:trigger x in res} {:trigger x in b[..]} {:trigger x in a[..]} :: x in a[..] && x in b[..] ==> x in res
  //   ENSURES: forall p: int, q: int {:trigger res[q], res[p]} :: 0 <= p < q < |res| ==> res[p] != res[q]
  //   ENSURES: forall p: int, q: int {:trigger res[q], res[p]} :: 0 <= p < q < |res| ==> exists i: int, j: int {:trigger a[j], a[i]} :: 0 <= i < j < a.Length && a[i] == res[p] && a[j] == res[q]
  {
    var a := new int[1] [25];
    var b := new int[0] [];
