    ...
    ensures res == Power(x, n) % m
{
    ...
    var mx: nat := x; // helper: remaining base for computing Power(x, n) (ghost)
    var p : nat := 1; // helper: partial result for computing Power(x, n) (ghost)
...
method TestsForFastExponentiation()
{
  ...
  //   POST Q1: n != 0
  //   POST Q2: p == x * Power(x, n - 1)
  {
    var x := 10;
    var n := 10;
    var p := FastExponentiation(x, n);
    expect p == 10000000000;
  }
...
method Main()
{
  ...
  TestsForFastModularExponentiation();
  print "TestsForFastModularExponentiation: all non-failing tests passed!\n";
}
