...
// Source: C:\Dados\Dafny\DafnyTestGen\test\verifixer\original\nitwit_tmp_tmplm098gxz_nit.dfy
// Method: nit_increment
// Generated: 2026-04-22 21:36:08
...
method TestsFornit_increment()
{
  ...
  //   POST Q2: nitness(b, carry)
  {
    ...
    expect sum == 0; // observed from implementation
  ...
  //   POST Q4: carry < b
  {
    ...
    var n := 1;
    var sum, carry := nit_increment(b, n);
    expect sum == 1 || sum == 0 || sum == 1 || sum == 0;
    expect carry == 1 || carry == 0 || carry == 0 || carry == 1;
    expect sum == 0; // observed from implementation
    expect carry == 1; // observed from implementation
  }
  ...
  //   POST Q2: nitness(b, carry)
  {
    ...
    expect nitness(b, sum);
    expect nitness(b, carry);
    expect sum == 1; // observed from implementation
...
method TestsForbibble_add()
{
  ...
  {
    var b := 8;
    var p: seq<nat> := [7, 5, 7, 7];
    var q: seq<nat> := [7, 7, 7, 7];
    var r := bibble_add(b, p, q);
    expect bibble(b, r);
    expect r == [7, 5, 7, 6]; // observed from implementation
  }
  ...
  //   POST Q1: bibble(b, r)
  {
    ...
    var r := bibble_add(b, p, q);
    expect bibble(b, r);
    expect r == [1, 1, 0, 0]; // observed from implementation
  }
  ...
  {
    var b := 7;
    var p: seq<nat> := [6, 5, 3, 5];
    var q: seq<nat> := [3, 3, 4, 4];
    var r := bibble_add(b, p, q);
    expect bibble(b, r);
    expect r == [3, 2, 1, 2]; // observed from implementation
  }
...
method TestsForbibble_increment()
{
  ...
  {
    var b := 9;
    var p: seq<nat> := [2, 2, 2, 8];
    var r := bibble_increment(b, p);
    expect bibble(b, r);
    expect r == [2, 2, 3, 0]; // observed from implementation
  }
  ...
  {
    var b := 9;
    var p: seq<nat> := [6, 4, 3, 0];
    var r := bibble_increment(b, p);
    expect bibble(b, r);
    expect r == [6, 4, 3, 1]; // observed from implementation
  }
...
method TestsForbibble_flip()
{
  ...
  {
    var b := 10;
    var p: seq<nat> := [6, 6, 6, 9];
    var fp := bibble_flip(b, p);
    expect bibble(b, fp);
    expect fp == [3, 3, 3, 0]; // observed from implementation
  }
  ...
  //   POST Q1: bibble(b, fp)
  {
    ...
    var fp := bibble_flip(b, p);
    expect bibble(b, fp);
    expect fp == [0, 0, 0, 2]; // observed from implementation
  }
  ...
  {
    var b := 10;
    var p: seq<nat> := [4, 9, 9, 3];
    var fp := bibble_flip(b, p);
    expect bibble(b, fp);
    expect fp == [5, 0, 0, 6]; // observed from implementation
  }
...
method TestsForn_complement()
{
  ...
  {
    var b := 9;
    var p: seq<nat> := [3, 4, 5, 6];
    var com := n_complement(b, p);
    expect bibble(b, com);
    expect com == [5, 4, 3, 3]; // observed from implementation
  }
