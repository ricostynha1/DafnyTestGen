...
// Source: C:\Dados\dafny\DafnyTestGen\test\Correct_progs2\in\task_id_644.dfy
// Method: ReverseUptoK
// Generated: 2026-03-21 23:17:58
...
method Passing()
{
  ...
  // Test case for combination {1}/Bs=3,k=1:
  //   PRE:  0 <= k <= s.Length
  //   POST: forall i :: 0 <= i < k ==> s[i] == old(s[k - 1 - i])
  //   POST: forall i :: k <= i < s.Length ==> s[i] == old(s[i])
  {
  ...
  // Test case for combination {1}/Bs=3,k==s:
  //   PRE:  0 <= k <= s.Length
  //   POST: forall i :: 0 <= i < k ==> s[i] == old(s[k - 1 - i])
  //   POST: forall i :: k <= i < s.Length ==> s[i] == old(s[i])
  {
