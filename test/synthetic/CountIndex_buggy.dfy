// OUT OF SCOPE under default options: mutation is in `FooPreCompute`
// (line 65: `while CountIndex != a.Length + 1` → `while CountIndex != CountIndex + 1`,
// always-true → infinite loop). The method has only `requires a.Length == b.Length`
// and no `ensures`, so DafnyCBT skips it under default options (no test
// objective). With --smoke-tests / -st, FooPreCompute is included and the
// mutant is killed (4 FAIL on infinite-loop timeout). This file is kept
// out_of_scope/ to reflect the default-options corpus-comparison run; flip
// to --smoke-tests to bring it back in scope.
