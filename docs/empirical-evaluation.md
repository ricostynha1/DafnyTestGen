...
After post-comparison fixes to four DafnyCBT bugs/gaps surfaced by this study — a soundness gap in the SMT encoding of set/multiset/map literals; a method-discovery gap that missed methods declared inside named modules; the previously-missing support for non-enum / single-self-recursive algebraic datatypes (now emitted as native Z3 `(declare-datatypes …)`); and module-qualified instance construction so class methods inside named modules can be tested — the head-to-head numbers on the intersection of programs both tools could exercise (55 programs) are below.

The intersection is **55** programs rather than the 57 `dafny generate-tests` runs tests for. The two excluded programs are both mutations of the same DafnyBench source file — a primality checker named `test_prime(i: nat) returns (result: bool) ensures result == IsPrime(i)`. DafnyCBT's auto-discovery skips any non-ghost method whose name contains the substring `test` or `Test` (case-insensitive), on the assumption that such methods are user-written test drivers — common in DafnyBench files that ship with example unit tests. This heuristic mis-classifies `test_prime` (where `test` is the verb meaning "to check", not "this is a test"), so the method is silently dropped before any SMT work. `dafny generate-tests` has no such name filter and treats it as an ordinary method. To keep the comparison apples-to-apples, both files are dropped from the intersection; this is a documented parity gap (refining the heuristic — e.g. requiring `Test` capitalised, or `_test`/`test_` boundaries — would close it without disturbing the existing test-driver detection):
...
### Wall-clock comparison on the intersection

Per-program `gen=Xs check=Ys` times, summed across the 55 intersection programs.

> **Caveats** (matter when reading the absolute numbers):
> - The two tools ran on **different platforms**: `dafny generate-tests` on WSL/Linux (the Boogie model-parser bug blocks it on Windows native), DafnyCBT on Windows native. Same hardware host, different OS overhead. Treat *relative* numbers as reliable, *absolute* seconds with a grain of salt.
> - The two tools emit **different test counts** by design — gen-tests produces ~1–3 tests per uncovered basic block, DafnyCBT up to 10 tests per method (the `--min-tests` budget). So *total seconds per program* mixes "per-test cost" with "test count chosen by the tool's strategy."
