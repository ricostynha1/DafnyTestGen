...
**Dead-clause pruning (`--dead-clause-pruning`, default off).** A DNF clause's *plain* (no-tier) combination being definitively Z3-UNSAT means every boundary/categorical tier of that clause is UNSAT too — a tier only *adds* constraints to an already-unsatisfiable formula. This is already exploited *within* a Phase via `baseUnsatMasks`, but that set was per-`SolveRange`-call, so the fact was relearned (re-solved) in each of Phase 1 / 2 / 2b. The flag persists it across the passes (keyed by the phase-stable `(preIdx, postMask)`), so a clause proven dead once is skipped everywhere. The canonical case: inlining a recursive function's base case yields a branch (`Sum2(v,k,i+1)` → `if k==i+1 then 0 else …`) that the spec's own bound makes unreachable (`k ≤ i ⟹ k ≠ i+1`) — clause {1} is structurally UNSAT, yet ~25–34 of its tier sub-combinations are re-solved per phase (SeqMaxSum). It is **monotone, not output-neutral**: it never removes a test (pruned solves were provably UNSAT → produced nothing), but the freed solve budget lets the budget-bounded Phase 3 round-robin reach additional `/R` repeats within the same `-n`/timeout (observed: `maxVal` gains a `{1}/R4`). So it is gated and behaviour-affecting (corpus kill-regression before any default-on), even though the effect is strictly non-negative. ~⅓ faster on fold-heavy methods.
