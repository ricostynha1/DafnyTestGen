...
**Bounded-fold closed-form encoding (`--bounded-fold`, default off).**

The runtime-`expect` fallback above is *sound but blind*: it generates inputs without a real objective, so it only kills a recursive-fold mutant when the discriminating partition is broad enough to be hit by chance. When the partition is narrow (e.g. `BelowZero`: "a negative element exists but no prefix sum is negative", witness `[5,-3]`), no budget kills it. `--bounded-fold` instead recognises a fold **structurally, at the AST level** ([BoundedFold.cs](../DafnyCBT/BoundedFold.cs) — never regex on surface syntax, because the decision must bind the recursive call's actuals to the formals and tell a slice from a variable) and emits an exact **bounded closed form** so the literal becomes a real SMT objective.
...
- **Conditional-contribution folds** — `Count(hi,s) = if hi==0 then 0 else if s[hi-1]%2==0 then 1+Count(hi-1,s) else Count(hi-1,s)` (nested-ITE body, the "CountPred" shape) — recognisable in principle but unimplemented; such mutants are anyway often killed by the broad-partition fallback.
...
**Phase 4 — precondition-only diversity fill (`--precond-fill`, default off).** When the targeted phases exhaust the postcondition's distinct *satisfiable-witness* space, the per-method budget is left under-filled — `segSumaMaxima2` yields only ~4–6 distinct tests at `-n 20` (dead clause {1}; the optimality+fold clause {2} has a small, Z3-clustered model region). The legacy choices were both poor: pad to `minTests` with near-duplicate clustered repeats (low value; the SeqMaxSum kill stays a ⅔ lottery), or stop short (budget wasted). Phase 4 adds the principled third option — a *robustness-testing* complement to partition testing: solve **precondition-only** queries (no post literals) with anti-trivial bias and accumulating input-fingerprint exclusions, emitting each input with the **full postcondition as a runtime `expect`**. Because Dafny functions are executable, that expect evaluates `Sum2`/`SumMaxToRight2` concretely, so it is a sound oracle: under the correct-spec assumption a precondition-valid input passes on the correct program and fails on a mutant *only* when discriminating — Phase 4 can only **add** tests and **never** produces a false kill. To stop Phase 3 from padding the whole budget with clustered repeats (which would prevent Phase 4 from triggering), `--precond-fill` reserves a slice: `phase3Target = max(post-Phase-2b count, minTests − ⌈minTests/3⌉)`, taken from Phase 3's *repeats only* (the targeted Phase 1/2/2b tests are never reduced); Phase 4 then fills `phase3Target … minTests`. No emitter change is needed — Phase 4 tests carry no `__unique__=="true"`, so they route to `fullPostconditionStrings`. Effect on `segSumaMaxima2`: the flaky ~⅓ kill becomes a **deterministic** kill (6–7 of the ~16 filled inputs FAIL, 4/4 runs), while the correct program stays 0-failing (0/0, sound). It only helps when the precondition is cheaply satisfiable (trivial for SeqMaxSum); methods whose *precondition itself* is the hard part (e.g. a relational prefix-sum-array `requires`) are a different cluster it does not address. Gated default-off pending corpus validation, like the other behaviour-affecting knobs.
