...
| `--skip-on-exception` | | In `--check` mode, tests whose implementation throws an unhandled exception (non-zero exit, no FAIL marker) are reported as `SKIP (exception from implementation)` instead of failing. Expects are commented out in the emitted test. Useful when the corpus contains methods that crash on boundary inputs and you want to separate those from genuine postcondition violations. Default: OFF |
