...
1. **Contradiction detection** — discards syntactically dead merges, never sending them to the solver. For each pair of relational literals on the same variable, the engine flags:
   ...
   - **Numeric range with no overlap**: `x op1 a ∧ x op2 b` where both RHSs parse as numeric constants and the implied lower / upper bounds cross. Computed by tracking `maxLower` and `minUpper` with their inclusive / exclusive flags. Equalities contribute simultaneously to both bounds (so `x == k` is treated as `x ∈ [k, k]`), which subsumes distinct numeric equalities as a special case. Catches `x > 5 ∧ x < 3`, `x >= 10 ∧ x <= 5`, `x == 0 ∧ x > 0`, `x == 1 ∧ x == 2` (empty intersection of `[1,1]` and `[2,2]`), etc. Symbolic RHSs are left alone (e.g., `x == y ∧ x == z` could coincide at `y == z`).
