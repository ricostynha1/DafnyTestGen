...
4. **Quantifier decomposition for boundary analysis.** Existential quantifiers `exists k :: lo <= k < hi && P(k)` can be opt-in decomposed (`--exists-decomposition` / `-ed`) into two mutually-exclusive cases (`P(lo)` and `!P(lo) ∧ ∃k>lo. P(k)`), mirroring the standard `A || B` ↦ `A`, `!A ∧ B` rule. Universal quantifiers rely on BVA size tiers (default 4: `|a|=0`, `|a|=1`, `|a|=2`, `|a|≥3` applied to every array/seq/set/multiset/map input and output; configurable via `--tiers N`) and the per-literal relevance check to force non-vacuous witnesses. These are combined with other contract clauses via cross-product.
