...
- **iset/map/imap input parameters** (`iset<T>`, `map<K,V>`, `imap<K,V>`). Note: these types as **return types** work fine when the input parameters are of supported types — the postcondition is used as the `expect` assertion and Dafny evaluates the expressions at runtime. `set<T>` and `multiset<T>` are now supported — see above
