...
| `--literal-bva` | `-lbva` | Phase 2 BVA: emit boundary + strict-companion tiers per relational post-clause literal `E1 op E2` (any expressions, not just bare variables). Includes chained-range mid-tier synthesis (`LO ≤ EXP ≤ HI` → `EXP=LO`, `EXP=HI`, `LO<EXP<HI`). Catches ROR-mutated boundary bugs whose witness lies strictly above/below a relational bound on a compound expression (e.g. `\|carPark\| > normalSpaces - badParkingBuffer`). |
