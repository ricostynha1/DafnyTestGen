...
4. **Quantifier decomposition for boundary analysis.** Existential quantifiers `exists k :: lo <= k < hi && P(k)` can be opt-in decomposed (`--exists-decomposition` / `-ed`) into three mutually-exclusive cases — `P(lo)`, `!P(lo) ∧ P(hi-1)`, and `!P(lo) ∧ !P(hi-1) ∧ ∃k :: lo<k<hi-1 ∧ P(k)` — exercising the witness at first / last / strictly-middle positions. The middle case forces witnesses Z3 wouldn't otherwise produce (Z3 defaults to the simplest model — typically the first or last index — so without it, mutants whose runtime divergence depends on a non-trivial interior witness escape). Universal quantifiers rely on BVA size tiers (default 4: `|a|=0`, `|a|=1`, `|a|=2`, `|a|≥3` applied to every array/seq/set/multiset/map input and output; configurable via `--tiers N`) and the per-literal relevance check to force non-vacuous witnesses. These are combined with other contract clauses via cross-product.
...
| `--exists-decomposition` | `-ed` | Enable 3-way mutually-exclusive decomposition of single-variable existentials (first / last / strictly-middle). Default OFF — quantifier kept as a single literal. |
