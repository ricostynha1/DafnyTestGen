...
In the above example, the cross-product of the two ensures clauses in DNF mode nominally yields 4 conjunctions. After canonicalisation and the two pruning passes, contradictory merges drop out and the surviving clauses simplify:
...
For `LastPosition`, `S = {Q4, Q5}` (guards `Q2`, `Q3` excluded, as well as `Q1`, as it refers only to inputs). The query forces `arr` to contain *multiple* duplicates of `elem` (for `Q4`) and at least one value different from `elem` (for `Q5`) so all literals are **simultaneously non-vacuous**: `Q4` (`arr[pos] == elem`) needs the existence of at least one value different from `elem`, `Q5` (`elem !∈ arr[pos+1..]`) needs at least one earlier copy to distinguish "last" from "first". Generated test:
...
| `--trust-unknown` | | Trust Z3 output values when the uniqueness check returns 'unknown' (default: false — safer). When true, concrete values are pinned even without a full uniqueness proof; when false, fall back to postcondition literals so the spec itself validates whatever the impl returns |
| `--drop-post-wf-guards` | | Drop well-formedness guards (e.g., `0<=i<a.Length`) generated while translating postcondition accesses (default: true). Prevents the bound from being re-asserted as a hard top-level fact and silently turning `a<b ==> a[i]==x` into `a<b && a[i]==x`, which broke uniqueness reasoning. Pass `--drop-post-wf-guards false` for legacy behaviour |
| `--skip-on-exception` | | In `--check` mode, tests whose implementation throws an unhandled exception (non-zero exit, no FAIL marker) are reported as `SKIP (exception from implementation)` instead of failing. The whole block is commented out in the emitted test (including the method call, so the crash doesn't abort later tests in the same wrapper) and prefixed with `// SKIPPED (exception from implementation): body commented out so the crash doesn't abort subsequent tests`. The results summary adds e.g. `, 2 skipped (exception)`. Useful when the corpus contains methods that crash on boundary inputs and you want to separate those from genuine postcondition violations. Default: OFF |
| `--comment-uncompilable` | | In `--check` mode, when `dafny build` fails due to uncompilable `expect` expressions (e.g. unbounded quantifiers, `old()` leaking into non-ghost contexts), automatically comment out the offending `CheckExpect` lines in `check_all.dfy`, mark matching `expect`s in the user-visible Tests.dfy with `// UNCOMPILABLE (...)`, and retry the build up to 3 times. Default: OFF — the check phase fails hard on build errors so users notice them. |
...
| `--reverse-bva-order` | `-rbva` | Run Phase 2b (categorical type/size coverage) before Phase 2 (refined-range BVA) instead of after. Default order is 2 → 2b. Phase 2's per-clause dedup against Phase 2b's keys is dropped under reverse, but solve-time subsumption still skips redundant entries. Useful for kill-curve ablation experiments comparing the two orderings |
