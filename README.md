...
4. **Quantifier coverage at boundary positions.** For each existential `exists k :: lo <= k < hi && P(k)` (and the equivalent negated-forall) in a postcondition, Phase 2 BVA emits three boundary tiers — `/Eb<n>=lo`, `/Eb<n>=hi`, `/Eb<n>=mid` — that pin the witness at the first / last / strictly-middle index respectively, with a mutex chain (`¬P[lo]` on `=hi`, `¬P[lo] ∧ ¬P[hi]` on `=mid`) forcing distinct witnesses across the three tiers. Each is a single SMT query; the original existential stays in the clause, so no DNF inflation. Catches mutants whose runtime divergence depends on a non-trivial interior witness (e.g. linear-search returning `-(n+1)` instead of `n+1` — only fails when n ≥ 1, which requires the searched element at a non-first/non-last position). Universal quantifiers rely on BVA size tiers (default 4: `|a|=0`, `|a|=1`, `|a|=2`, `|a|≥3` applied to every array/seq/set/multiset/map input and output; configurable via `--tiers N`) and the per-literal relevance check to force non-vacuous witnesses.
...
| `--no-literal-bva` | `-nlbva` | Disable the default literal-centric Phase 2 BVA and fall back to the legacy variable-centric extractor. Default ON: for each relational post-clause literal `E1 op E2` emit boundary (`E1 = E2`) and strict-companion (`E1 > E2` / `E1 < E2`) tiers, plus chained-range mid synthesis (`LO ≤ EXP ≤ HI` → `EXP=LO` / `EXP=HI` / `LO<EXP<HI`). The variable-centric variant emits boundary tiers per int/nat *variable* extracted from the contract — narrower coverage (misses bounds on compound expressions like `\|carPark\| > K`) but slightly fewer schedule entries. |
